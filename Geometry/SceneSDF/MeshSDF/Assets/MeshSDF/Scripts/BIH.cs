        ...
        public static int HoarePartition(IBounded[] elements, int start, int end, float pivot, ref BIHNode node, int axis)
        {
            ...
            while (i <= j)
            {
                ...
                if (i <= j)
                {
                    ...
                    Utils.Swap(ref elements[i++], ref elements[j--]);
                }
            }
                                              ...
                                              in Vector3 point)
        {
            ...
            for (int i = node.start; i < node.start + node.count; ++i)
            {
                Triangle t = triangles[i];
                    ...
                    Utils.NearestPointOnTri(in vertices[t.i1],
                                               in vertices[t.i2],
                                               in vertices[t.i3],
                                               in point,
                                               out pointOnTri);
                ...
                if (sqrDistance < minDistance)
                {
                    ...
                    Utils.BarycentricCoordinates(in vertices[t.i1], in vertices[t.i2], in vertices[t.i3], in pointOnTri, ref bary);
                    Utils.BarycentricInterpolation(in normals[t.i1],
                                                      in normals[t.i2],
                                                      in normals[t.i3],
                                                      in bary,
                                                      out interpolatedNormal);

                    sign = Utils.PureSign(pointToTri.x * interpolatedNormal.x +
