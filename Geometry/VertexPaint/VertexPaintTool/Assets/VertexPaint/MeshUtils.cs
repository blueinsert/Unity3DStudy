...
namespace bluebean
{
    public static class MeshUtils
    {
                                                 ...
                                                 ref Vector3 normal)
        {
            ...
            VectorMath.Subtract(vert0, vert1, ref tv1);
            VectorMath.Subtract(vert0, vert2, ref tv2);

            VectorMath.Cross(dir, tv2, ref tv4);
            det = Vector3.Dot(tv1, tv4);

            if (det < Mathf.Epsilon)
                return false;

            VectorMath.Subtract(vert0, origin, ref tv3);
            ...
            VectorMath.Cross(tv3, tv1, ref tv4);
            ...
            VectorMath.Cross(tv1, tv2, ref normal);
        ...
        */
        public static bool WorldRaycast(Ray InWorldRay, Matrix4x4 transform, Vector3[] vertices, int[] triangles, out RaycastHit hit, float distance = Mathf.Infinity)
        {
            Ray ray = InWorldRay;
            if (transform != null)
            {
        ...
        */
        public static bool MeshRaycast(Ray InRay, Vector3[] vertices, int[] triangles, out RaycastHit hit, float distance = Mathf.Infinity)
        {
            ...
            hit = new RaycastHit(Mathf.Infinity,
                                    Vector3.zero,
                                    Vector3.zero,
                                    -1);
            /**
