	...
	public class CrossSectionEditor : Editor
	{
		...
		public override void OnInteractivePreviewGUI(Rect region, GUIStyle background)
		{
			...
			for (int i = 0; i < section.Segments; i++){
				...
				Vector2 olsPos = pos;
				selected[i] = DraggableIcon.Draw(selected[i],i,ref pos,Color.red);
				...
				if (pos != olsPos){
					...
					pos.x = Mathf.Clamp(CrossSection.SnapTo(pos.x - region.center.x,section.snapX,5) / (region.width * 0.5f),-1,1);
					pos.y = Mathf.Clamp(CrossSection.SnapTo(pos.y - region.center.y,section.snapY,5) / (region.height * 0.5f),-1,1);
