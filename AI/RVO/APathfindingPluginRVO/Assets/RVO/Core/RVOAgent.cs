	...
	public class Agent : IAgent {
		...
		internal struct VO {
			...
			public static VO SegmentObstacle (Vector2 segmentStart, Vector2 segmentEnd, Vector2 offset, float radius, float inverseDt, float inverseDeltaTime) {
				...
				var closestOnSegment = Math.ClosestPointOnSegment(segmentStart, segmentEnd, Vector2.zero);
				...
				if (closestOnSegment.magnitude <= radius) {
					...
					var normal1 = -Math.ComplexMultiply(segmentStart, new Vector2(radius, Mathf.Sqrt(Mathf.Max(0, startSqrMagnitude - radius*radius)))) / startSqrMagnitude;
					var endSqrMagnitude = segmentEnd.sqrMagnitude;
					var normal2 = -Math.ComplexMultiply(segmentEnd, new Vector2(radius, -Mathf.Sqrt(Mathf.Max(0, endSqrMagnitude - radius*radius)))) / endSqrMagnitude;
					...
					if (det1 >= 0 && det2 >= 0) {
						...
						if (Vector2.Dot(p - line1, dir1) > 0 && Vector2.Dot(p - line2, dir2) < 0) {
							if (segment) {
								...
								if (det3 < radius) {
									var closestPointOnLine = (Vector2)Math.ClosestPointOnSegment(segmentStart, segmentEnd, p);
									var dirFromCenter = p - closestPointOnLine;
									float distToCenter;
									gradient = Math.Normalize(dirFromCenter, out distToCenter);
