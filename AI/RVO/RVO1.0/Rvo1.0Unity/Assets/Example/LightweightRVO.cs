		...
		public void CreateAgents (int num) {
			...
			if (type == RVOExampleType.Circle) {
				...
				for (int i = 0; i < agentCount; i++) {
					...
					agents.Add(agent);
					goals.Add(-pos);
					colors.Add(Math.HSVToRGB(i * 360.0f / agentCount, 0.8f, 0.6f));
				}
				...
				for (int i = 0; i < agentCount; i++) {
					...
					colors.Add(Math.HSVToRGB(targetAngle * Mathf.Rad2Deg, 0.8f, 0.6f));
				}
				...
				for (int i = 0; i < agentCount; i++) {
					...
					colors.Add(Math.HSVToRGB(angle * Mathf.Rad2Deg, 0.8f, 0.6f));
				}
			}
