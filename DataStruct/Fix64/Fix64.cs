...
namespace FixMath.NET {
    ...
    public partial struct Fix64 : IEquatable<Fix64>, IComparable<Fix64> {
        ...
        /// <summary>
        /// 实际是LutInterval(查找表每个带宽占据的角度范围)的倒数
        /// </summary>
        ...
        const long PI_TIMES_2 = 0x6487ED511;//==2pi*2^32
        const long PI = 0x3243F6A88;
        const long PI_OVER_2 = 0x1921FB544;//==pi/2*2^32
        ...
        public static Fix64 operator /(Fix64 x, Fix64 y) {
            ...
            while (remainder != 0 && bitPos >= 0) {
                ...
                //放大被除数
                remainder <<= shift;
                //会导致结果和余数也变大，加上这个偏移，应用于最后结果进行修正
                bitPos -= shift;
            ...
            //第一次的整除得到整数应该是左移32位，实际移动33位，需要右移一位
            // rounding
            //小数精度末尾会舍去一位，如果末尾为1，加一后，倒数第二位（舍去后的倒数第一位）加一，即向上取近似
            //否则直接忽略
            ++quotient;
